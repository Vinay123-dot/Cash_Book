...
const EditModalInDayBook = (props) => {
    ...
    const { dayBookObj,handleCancelDBook,handleSaveEditDayBook,requiredArrayData } = props;
    ...
    if (!dayBookObj.showEditDaybookModal) return null;
    ...
    const handleChangeSalesType = (name,sValue,setFieldValue,sArr) => {
        ...
        let temp = [];
        if (sValue == 1) {
            temp = requiredArrayData.paymentListInfo.filter((eachDoc) => eachDoc.Id != 4);
        } else {
            temp = JSON.parse(JSON.stringify(requiredArrayData.paymentListInfo));
        }
    }
    ...
    return (
        <Modal openModal={true} height={"90%"} width={"90%"}  ai = {null}>
            <>
                <Formik
                    initialValues = {dayBookObj.editDayBookObj}
                            ...
                            <Form style={{ overflow: "auto"}}>
                                ...
                                <div className="grid grid-cols-1 gap-10 px-4 py-2 lg:grid-cols-3 md:grid-cols-2">
                                    ...
                                    <AntdFormikSelect
                                        ...
                                        name = "sales_type"
                                        ph = "--Select Sale Type--"
                                        handleChange = {(name, selectedValue) => handleChangeSalesType(name, selectedValue, setFieldValue, requiredArrayData.salesType)}
                                        Arr = {requiredArrayData.salesType}
                                    />
                                    <AntdInput
                                        text="Bill Number"
                                        value='bill_no'
                                    ...
                                    />
                                    {
                                        showSelectBox("Customer Type", "customer_type", "--Select CustomerType--", getCustomerList(requiredArrayData.customerListInfo, values), setFieldValue)
                                    }
                                    {
                                ...
                                </div>
                                {
                                    ...
                                    <>
                                        <CashTypes
                                            ...
                                            valObj = {values}
                                            paymentListInfo = {requiredArrayData.paymentListInfo}
                                            upiTypeInfo = {requiredArrayData.upiTypeInfo}
                                        />
